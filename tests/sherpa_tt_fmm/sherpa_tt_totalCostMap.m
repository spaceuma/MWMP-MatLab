...
costMap(dilatedObstMap==1)= minCost + auxMap((dilatedObstMap==1))./min(min(auxMap((dilatedObstMap==1))));
...
% gBCMx = gBCMx.*abs(gBCMx);
% gBCMy = gBCMx.*abs(gBCMy);
...
while 1   
    ...
    Ox = zeros(size(Q,1),size(t,2)); %Obstacles cost
    for i = 1:size(t,2)
        [Ox(7,i), Ox(8,i)] = getGradientTotalCost(x(15,i), x(16,i), mapResolution, gBCMx, gBCMy);
        Ox(7,i) = Ox(7,i)*0.06;
        Ox(8,i) = Ox(8,i)*0.06;
    end
...
%     xh(6,1) = 0;
    ...
    for i = size(t,2)-1:-1:1
        ...
        s(:,:,i) = A(:,:,i).'*(eye(size(Q,1)) - P(:,:,i+1)*M(:,:,i)*B(:,:,i)*inv(R)*B(:,:,i).')*s(:,:,i+1)+...
            A(:,:,i).'*P(:,:,i+1)*M(:,:,i)*B(:,:,i)*uh0(:,i) - Q(:,:,i)*xh0(:,i) + Ox(:,i);
    end
        ...
        for n = 1:size(alfa,2)
            ...
            for i = 1:size(t,2)-1
                J(n) = J(n) + 1/2*((x(:,i)-x0(:,i)).'*Q(:,:,i)*(x(:,i)-x0(:,i))...
                    + (u(:,i)-u0(:,i)).'*R*(u(:,i)-u0(:,i))) + getTotalCost(x(15,i), x(16,i), mapResolution, totalCostMap);
            end            
        end
        [mincost, ind] = min(J);
        alfamin = alfa(ind);
...
%         u = u + uh;
    ...
    plot3([TB0(1,4) TB1(1,4) TB2(1,4) TB3(1,4) TB4(1,4) TB5(1,4) TB6(1,4)]+x(15,1),...
          ...
          [TB0(3,4) TB1(3,4) TB2(3,4) TB3(3,4) TB4(3,4) TB5(3,4) TB6(3,4)]+zB0, 'Color', 'r', 'LineWidth', 2.5);
    hold on;
    ...
    contourf(X,Y,dilatedObstMap) 
    plot3(x(15,:),x(16,:),zB0*ones(1,size(t,2)), 'LineWidth', 5)
