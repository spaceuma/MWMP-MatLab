...
while 1   
    ...
    for i = 2:size(t,2)
        ...
        x(4:9,i) = x(4:9,i-1) + Jac(:,:,i-1)*x(15:20,i-1)*dt; 
        x(10:11,i) = x(10:11,i-1) + x(13:14,i-1)*dt;    
        x(12,i) = x(12,i-1); 
        x(13:20,i) = x(13:20,i-1) + u(:,i-1)*dt; 
    end
...
%     xh(6,1) = 0;
    ...
    if norm(uh)<0.01*norm(u) || ((norm(uh)<0.08*norm(u))&&(endDist<0.05))
        ...
        for n = 1:size(alfa,2)
            ...
            for i = 1:size(t,2)-1
                J(n) = J(n) + 1/2*((x(:,i)-x0(:,i)).'*Q(:,:,i)*(x(:,i)-x0(:,i))...
                    + (u(:,i)-u0(:,i)).'*R*(u(:,i)-u0(:,i))) + getTotalCost(x(10,i), x(11,i), mapResolution, totalCostMap);
            end            
        end
        [mincost, ind] = min(J);
        alfamin = alfa(ind);
...
%         u = u + uh;
    ...
    hold on;
    [TB0, TB1, TB2, TB3, TB4, TB5, TB6] = direct(q(:,end-1));
